...
with open(file_to_save, "w") as txt_file:
    ...
    for county in county_votes_dict: 
        ...
        county_vote_count = county_votes_dict[county]
        # 6c: Calculate the percent of total votes for the county.
        vote_percentage = float(county_vote_count) / float(total_votes) * 100

         # 6d: Print the county results to the terminal.
        county_results = (
        f"{county}: {vote_percentage:.1f}% ({county_vote_count:,})")
       
        print(county_results)
         # 6e: Save the county votes to a text file.
        txt_file.write(county_results+"\n")
         # 6f: Write a decision statement to determine the winning county and get its vote count.
        if county_vote_count > county_votes_largest_turnout:

            county_largest_turnout = county
            county_votes_largest_turnout = county_vote_count
